...
{
    // 2/4/2019 Added elevator chirp in FBWA mode
    if (t0_fbwa == 0) {
    ...
    if (t_in_fbwa < 25*1000) {
        ...
        if (t_in_fbwa < 1000*2*3.14159/(2*wmin)) { // First half period, 1 s
            ...
            d_elevator = 0.5*450*sinf(theta_sine);
            SRV_Channels::set_output_scaled(SRV_Channel::k_elevator, k_elevator);
        }
        if (t_in_fbwa >= 1000*2*3.14159/(2*wmin)) {
            K_sine = 0.0187*(expf(4*(t_in_fbwa/1000)/Trec)-1);
			omega_rps = wmin+K_sine*(wmax-wmin);
			theta_sine += omega_rps*time_step_*0.001;
            d_elevator = 450*sinf(theta_sine);
            SRV_Channels::set_output_scaled(SRV_Channel::k_elevator, d_elevator);
        }
